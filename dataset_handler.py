...
class YOLO_Dataset(Dataset):
    ...
    * Предоставляет возможность найти статистику по указанной части датасета и позволяет проверить нормализацию.
    ...
    def __init__(
        ...
        **transform_kwargs,
        ...
        if not self.no_yolo:
            ...
            paths = {
                dataset_part: join(dataset_dir, data[dataset_part].strip("./"))
                for dataset_part in ["train", "val", "test"]
            }
            self.images_paths = {
                part: np.array(
                    [
                        join(paths[part], img_name)
                        for img_name in os.listdir(paths[part])
                    ]
                )
            ...
            self.images = np.array(
                [
                    join(self.dataset_dir, img_name)
    ...
    def change_part(self, part: Literal["train", "val", "test", "all"]):
        ...
        if part == "all":
            self.images = np.concatenate(
                [self.images_paths[key] for key in self.images_paths.keys()], axis=0
            )
        else:
    ...
    def check_normalization(self, mean: ArrayLike = None, std: ArrayLike = None):
        ...
        assert (
            cur_mean is not None and cur_std is not None
        ), "Необходимо сначала рассчитать статистику!"

        print(f"Текущее mean = {cur_mean}")
        print(f"Текущее std = {cur_std}")

        mean, std = self.find_statistic(
            self._update_transform(cur_mean, cur_std), save=False
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--delete_duplicates", action="store_true", help="Удалить дубликаты"
    )
    parser.add_argument(
        "--find_statistic", action="store_true", help="Рассчитать статистику"
    )
    parser.add_argument(
        "--check_normalization",
        action="store_true",
    ...
    dataset = YOLO_Dataset(
        dataset_dir=args.path, part=args.part, no_yolo=not args.yolo_dir
    )
